...
def validate_unique_phone(phone):
    account = Account.objects.filter(phone_number=phone)
    if account.exists():
        return True
    return False
    ...
    def post(self, request):
        if request.method == "POST":
            ...
            if validate_username(username) != None:
                ...
                data['response'] = 'Error'
                return Response(data)
            phno = request.data.get('phone_number')
            
            if validate_unique_phone(phno):
                err["phone_number"] = "Phone Number is Unique"
                err['error_code'] = 404
                # raise serializers.ValidationError('This field must be an integer value.')
                return Response(err)
            serializer = AccountSerializer(data=request.data)
            #because of phone number unique, there can have error
            
            if serializer.is_valid(raise_exception=ValueError):
                account = serializer.create(validated_data=request.data)
                data['response'] = "Successfully register new user"
                #for blogpost api in react adding id
                data["id"] = account.id
